...
import { Contest, contestSchema, Problem, problemSchema } from "./types.ts";
...
async function parseResponse(...query: string[]) {
  const response = responseSchema.parse(
    await $`oj-api ${query}`.stderr("null").json(),
  );
...
export async function ojTest(
  problem: Problem,
  exec: string[],
  buffer: (line: string) => Promise<void>,
): Promise<boolean> {
