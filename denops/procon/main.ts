...
export async function main(denops: Denops): Promise<void> {
  ...
  await denops.cmd(
    `command! -nargs=1 ProconDownload call denops#notify("${denops.name}", "proconDownload", [<f-args>])`,
  );
  await denops.cmd(
    `command! ProconTest call denops#notify("${denops.name}", "proconTest", [])`,
  );
  await denops.cmd(
    `command! -bang ProconSubmit call denops#notify("${denops.name}", "proconSubmit", [<q-bang>])`,
  );
  await denops.cmd(
    `command! ProconBrowse call denops#notify("${denops.name}", "proconBrowse", [])`,
  );
...
async function readProblem(denops: Denops): Promise<Problem> {
  return problemSchema.parse(yaml.parse(
...
async function prepareDir(denops: Denops, contest: Contest): Promise<void> {
  const contestDir = contest.url.replace(
    /^https:\/\/(\w+)\..+\/([^/]+)$/,
    "$1/$2",
  );
